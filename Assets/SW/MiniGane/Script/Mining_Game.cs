...
public class Mining_Game : MonoBehaviour
{
    ...
    */
    ...
    private void ShallowDig()
    {
        ...
        //-- 마우스 우 클릭 얖게 파기
        /* 얇게 파기란 한칸을 파는걸 의미 한다.
         * 만약 얇게 파기를 실행했으나 그대로이면 그 부분은 아무것도 없는 흙인 부분이다. 만약 얇게 파기 실행 시 한칸이 겉어지고 뿌리가 있으면 그 부분 1칸만 흙을 치우고 뿌리를 노출시킨다.
         */
    ...
    private void DeepDig()
    {
        ...
        for (int i = 0; i < directions.GetLength(0); i++)
        {
            ...
            {
                cell.revealed = true;
                state[cellPosition.x + xOffset, cellPosition.y + yOffset] = cell;
            }
        }
        board.Draw(state);
    }
    ...
    private void GenerateStartMine()
    {
        ...
        {
            //--{4,5~8} 좌 시작 지점 생성
            state[4, resultYPos].type = Cell.Type.StartPlant;
            state[3, resultYPos].type = Cell.Type.StartPlant;
        ...
        {
            //--{9,5~8} 우 시작 지점 생성
            state[9, resultYPos].type = Cell.Type.StartPlant;
            state[10, resultYPos].type = Cell.Type.StartPlant;
        ...
        {
            //--{5~8,4} 상 시작 지점 생성
            state[resultXPos, 4].type = Cell.Type.StartPlant;
            state[resultXPos, 3].type = Cell.Type.StartPlant;
        ...
        {
            //--{5~8,9} 하 시작 지점 생성
            state[resultXPos, 9].type = Cell.Type.StartPlant;
            state[resultXPos, 10].type = Cell.Type.StartPlant;
        ...
        foreach (var item in startPlantPos)
            ExpansionMine(item.x, item.y, item.z);
    ...
    private void ExpansionMine(int posX, int posY, int expansionLengh)
    {
        ...
        //--검사 시작
        if (IsValid(posX, posY + 1) && state[posX, posY + 1].type == Cell.Type.Empty) //--상 
            expansionPos.Add(new Vector2Int(posX, posY + 1));

        if (IsValid(posX, posY - 1) && state[posX, posY - 1].type == Cell.Type.Empty) //--하
            expansionPos.Add(new Vector2Int(posX, posY - 1));

        if (IsValid(posX - 1, posY) && state[posX - 1, posY].type == Cell.Type.Empty) //--좌
            expansionPos.Add(new Vector2Int(posX - 1, posY));

        if (IsValid(posX + 1, posY) && state[posX + 1, posY].type == Cell.Type.Empty) //--우
            expansionPos.Add(new Vector2Int(posX + 1, posY));
        ...
        state[expansionPos[randomPos].x, expansionPos[randomPos].y].number = expansionLengh;
        state[expansionPos[randomPos].x, expansionPos[randomPos].y].type = Cell.Type.Number;
